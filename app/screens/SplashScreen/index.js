...
class SplashScreen extends React.PureComponent {
              ...
              this.setState(
                {
                  ...
                  settingObj: response.data.setting
                ...
                () => {
                  ...
                  if (response.data.setting[Numbering.kHomeTitle]) {
                    this.setState({
                      appName: response.data.setting[Numbering.kHomeTitle],
                    });
                  }
                  this.getFile();
                },
  ...
  checkOffLineModeAvailable = async () => {
    ...
    if (localDataVer) {
      ...
      Alert.alert(
        'Thông báo',
        'Có lỗi trong quá trình tải dữ liệu ứng dụng về máy! (173)',
      );
    }
  };
    ...
    RNFetchBlob.fetch('GET', this.state.downloadLink, {
      ...
      .then(res => {
        ...
        if (status == 200) {
          ...
          Alert.alert(
            'Thông báo',
            'Có lỗi trong quá trình tải dữ liệu ứng dụng về máy! (200)',
          );
        }
      })
      .catch((errorMessage, statusCode) => {
        ...
        Alert.alert(
          'Thông báo',
          'Có lỗi trong quá trình tải dữ liệu ứng dụng về máy! (206)',
        );
      });
  };
    ...
    try {
      ...
      realm.write(() => {
        ...
        data.qAndA.forEach((element, index) => {
          ...
          element.textForSearch = nonAccentVietnamese(
      ...
      await saveData(
        Numbering.kHomeTitle,
        settingObj[Numbering.kHomeTitle]
          ? settingObj[Numbering.kHomeTitle].trim()
          : '',
      );
      await saveData(
        Numbering.kBannerLink,
        settingObj[Numbering.kBannerLink]
          ? settingObj[Numbering.kBannerLink].trim()
          : '',
      );
      await saveData(
        Numbering.kYoutubeLink,
        settingObj[Numbering.kYoutubeLink]
          ? settingObj[Numbering.kYoutubeLink].trim()
          : '',
      );
      await saveData(
        Numbering.kMlmCompanyListUpdateAt,
        settingObj[Numbering.kMlmCompanyListUpdateAt]
          ? settingObj[Numbering.kMlmCompanyListUpdateAt].trim()
          : '',
      );

      await saveData(
        Numbering.kAdsLink,
        adsSetting.link ? adsSetting.link.trim() : '',
      );
      await saveData(
        Numbering.kAdsContent,
        adsSetting.html ? adsSetting.html.trim() : '',
      );
      ...
      Alert.alert(
        'Thông báo',
        'Có lỗi trong quá trình tải dữ liệu ứng dụng về máy!(302)',
      );
    }
  };
