...
class QAndADetailScreen extends React.PureComponent {
  ...
  share = async () => {
    let message =
      'Hỏi: ' +
      Utils.removeHTMLTags(this.state.question) +
      '.\n Đáp: ' +
      Utils.removeHTMLTags(this.state.answer) +
      '.';
    try {
      await Share.share({
        title: 'iMLM',
        ...
        <View style={styles.stylePager}>
          ...
          <View style={[styles.stylePagerBody, Styles.styleHeaderCenter]}>
            <Text
              ...
              Câu hỏi số {this.state.questionNumber}
            </Text>
          </View>
          <View style={[styles.stylePagerEdge, styles.styleHeaderCenter]}>
  ...
  renderContent = () => {
    console.log(
      'renderContent generateDefaultTextStyles(this.state.fontSize) = ' +
        JSON.stringify(generateDefaultTextStyles(this.state.fontSize)),
    );
    return (
