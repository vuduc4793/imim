...
import {Colors, Styles, Icons, Texts, AppDimensions, Jsons} from "../../constants";
    ...
    this.setState({isLoading: true}, ()=>{
      Realm.open(databaseOptions).then(realm => {
        ...
        for (let p of data) {
          ...
          console.log("removeHTMLTags(item.shortName)", removeHTMLTags(p.shortName))
          dataList.push(p);
        }
    ...
    return (
      <View>
        <TouchableOpacity style={Styles.styleHeaderButtonTopLeft} onPress={this._goBack}>
          <RNImage source={Icons.IC_BACK} style={[Styles.styleHeaderImageTopLeft]} />
        </TouchableOpacity>
      </View>
    );
  }
    ...
    return (
      <View>
        <TouchableOpacity style={Styles.styleHeaderButtonTopLeft} onPress={this._openSearch}>
          <RNImage source={Icons.IC_FIND} style={[Styles.styleHeaderImageTopRight, {tintColor: 'white'}]} />
        </TouchableOpacity>
      </View>
    );
  }
                ...
                <View style={{flex: 1, alignItems: 'center', flexDirection: 'row'}}>
                  {
                    ...
                    (<Image
                      ...
                      PlaceholderContent={<Image source={Icons.IC_LAW_DOC} style={{height: 60, width: 60, borderRadius: 5, tintColor: Colors.colorPrimary }}/>}
  ...
  getData = () =>{
    ...
    if (keyword == ""){
      ...
      fullData.forEach(element => {
        if (nonAccentVietnamese(element.name).contains(nonAccentVietnamese(keyword))){
