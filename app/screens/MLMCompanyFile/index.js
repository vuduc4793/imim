...
class MLMCompanyFile extends React.PureComponent {
  constructor(props) {
    super(props);
  ...
  async componentDidMount(){
    ...
    getCompanyHoSo(id).then((response) => {
      this.setState({
        isLoading: false
      })
      console.log('CompanyFile getData response', JSON.stringify(response));
      ...
      if (obj['soap:Envelope'] 
          ...
          && obj['soap:Envelope']['soap:Body']['VccaDNBHDCHosoResponse']['VccaDNBHDCHosoResult']) {
            ...
            console.log('CompanyFile getData data', JSON.stringify(data))
            this.setState({
              companyData: data
            })
      }
    }).catch(err => {
      console.log('CompanyFile getData error', err);
    });
  }
              ...
              return (
                <View style={[Styles.sectionStyle, {justifyContent: 'center', width: '100%'}] }>
                  ...
                  <InfoLineComponent 
                    ...
                    info1={{'title': "Tên hồ sơ", 'content' : item.ten}}
                    info2={{'title': "Tệp", 'content': item.files}}
                  />  
                  {/* <View style={{height: 0.5, width: '100%', backgroundColor: Colors.colorBlack3}}/> */}
                  <InfoLineComponent 
                    style={{width: '100%', borderBottomColor: 'gray', borderBottomWidth: 0}}
                    info1={{'title': "Ngày gửi", 'content' : item.ngaygui}}
                    info2={{'title': "", 'content': ""}}
                  />
                </View>
              );
...
export default connect(mapStateToProps, mapDispatchToProps)(MLMCompanyFile);
